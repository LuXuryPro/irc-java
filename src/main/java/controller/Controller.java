...
public class Controller implements TreeSelectionListener, ActionListener {
    ...
    public Controller(View v, Model m) {
        ...
        this.main_thread = new Thread(() -> main_run());
    ...
    private void main_run() {
        while (true) {
            try {
                ArrayList<IRCEvent> ircEvents;
                model.waitIsConnected();
                ircEvents = this.model.getCurrentServer().getCurrentChannel()
                        .waitForEvents();
                this.update_gui(ircEvents);
    ...
    private void update_gui(final ArrayList<IRCEvent> ircEvents) {
        if (ircEvents == null)
            return;
        SwingUtilities.invokeLater(() -> {
            StringBuffer stringBuffer = new StringBuffer();
            stringBuffer.append("<body bgcolor='#1C1C1C'><font color = 'white' face = 'monospace'>");
            for (IRCEvent ircEvent : ircEvents)
                if (ircEvent.generateDisplayString() != null)
                    stringBuffer.append(ircEvent.generateDisplayString() + "<br>");
            stringBuffer.append("</font></body>");
            view.setText(stringBuffer.toString());
            view.getUsersList().clear();
            model.getCurrentServer().getCurrentChannel().getUsers().forEach(user ->
                    view.getUsersList().addUser(user.toString())
            );
            view.setTopic(model.getCurrentServer().getCurrentChannel().getTopic());
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                if (treeSelectionEvent.getNewLeadSelectionPath() != null)
                    if (treeSelectionEvent.getNewLeadSelectionPath().getPath().length == 2) {
                        String s = treeSelectionEvent.getNewLeadSelectionPath().getPath()[1]
                                .toString();
                        model.setCurrentServer(s);
                        model.getCurrentServer().setCurrentChannel(null);
                        main_thread.interrupt();
                    } else if (treeSelectionEvent.getNewLeadSelectionPath().getPath().length == 3) {
                        String s = treeSelectionEvent.getNewLeadSelectionPath().getPath()[1]
                                .toString();
                        String c = treeSelectionEvent.getNewLeadSelectionPath().getPath()[2]
                                .toString();
                        model.setCurrentServer(s);
    ...
    public void actionPerformed(final ActionEvent actionEvent) {
        ...
        SwingUtilities.invokeLater(() -> {
            String command = actionEvent.getActionCommand().trim();
